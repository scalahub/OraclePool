package oraclepool.v2a

import kiosk.encoding.ScalaErgoConverters.{getAddressFromErgoTree, getStringFromAddress}
import kiosk.ergo._
import kiosk.tx.TxUtil
import oraclepool.v2a.helpers.MockHelpers
...
class UpdateSpec extends PropSpec with Matchers with ScalaCheckDrivenPropertyChecks with HttpClientTesting with MockHelpers {
  ...
  def vote(newPoolErgoTree: Values.ErgoTree, ballotAddress: String, ballotTokenId: String, optRewardTokenId: Option[String] = None, optRewardTokenAmt: Option[Long] = None)(
    ...
    val votedBox1 = voteForUpdate(newPoolErgoTree.bytes, 1, ballotAddress, minStorageRent, ballotBox1, privKey1, 0, None, Some(ballotTokenId), optRewardTokenId, optRewardTokenAmt)
    val votedBox2 = voteForUpdate(newPoolErgoTree.bytes, 1, ballotAddress, minStorageRent, ballotBox2, privKey2, 0, None, Some(ballotTokenId), optRewardTokenId, optRewardTokenAmt)
    val votedBox3 = voteForUpdate(newPoolErgoTree.bytes, 1, ballotAddress, minStorageRent, ballotBox3, privKey3, 0, None, Some(ballotTokenId), optRewardTokenId, optRewardTokenAmt)
    val votedBox4 = voteForUpdate(newPoolErgoTree.bytes, 1, ballotAddress, minStorageRent, ballotBox4, privKey4, 0, None, Some(ballotTokenId), optRewardTokenId, optRewardTokenAmt)
    val votedBox5 = voteForUpdate(newPoolErgoTree.bytes, 1, ballotAddress, minStorageRent, ballotBox5, privKey5, 0, None, Some(ballotTokenId), optRewardTokenId, optRewardTokenAmt)
    val votedBox6 = voteForUpdate(newPoolErgoTree.bytes, 1, ballotAddress, minStorageRent, ballotBox6, privKey6, 0, None, Some(ballotTokenId), optRewardTokenId, optRewardTokenAmt)

    Array(votedBox1, votedBox2, votedBox3, votedBox4, votedBox5, votedBox6)
  }

  def refresh(oldPoolBox: InputBox, oldRefreshBox: InputBox, anyRewardTokenId: String, oracleTokenId: String, refreshNFT: String)(implicit ctx: BlockchainContext) = {
    val rewardTokenAmt = oldPoolBox.getTokens.get(1).getValue
    val oracleBox1 = bootstrapOracleBox(pubKey1, 10, Some(anyRewardTokenId), optOracleTokenId = Some(oracleTokenId))
    val oracleBox2 = bootstrapOracleBox(pubKey2, 20, Some(anyRewardTokenId), optOracleTokenId = Some(oracleTokenId))
    val oracleBox3 = bootstrapOracleBox(pubKey3, 30, Some(anyRewardTokenId), optOracleTokenId = Some(oracleTokenId))
    val oracleBox4 = bootstrapOracleBox(pubKey4, 40, Some(anyRewardTokenId), optOracleTokenId = Some(oracleTokenId))
    val oracleBox5 = bootstrapOracleBox(pubKey5, 50, Some(anyRewardTokenId), optOracleTokenId = Some(oracleTokenId))

    val dataPoint1 = createDataPoint(
      1,
      0,
      ...
      0,
      10,
      customRewardTokenId = Some(anyRewardTokenId),
      customOracleTokenId = Some(oracleTokenId)
    )
    val dataPoint2 = createDataPoint(
      1,
      0,
      contracts.oracleAddress,
      minStorageRent,
    ...
    val dataPoint3 = createDataPoint(
      1,
      0,
      contracts.oracleAddress,
      minStorageRent,
    ...
    val dataPoint4 = createDataPoint(
      1,
      0,
      contracts.oracleAddress,
      minStorageRent,
    ...
    val dataPoint5 = createDataPoint(
      1,
      0,
      contracts.oracleAddress,
      minStorageRent,
      ...
      .createTx(
        ...
        Array(
          KioskBox(
            poolAddress,
            minStorageRent,
            Array(KioskLong(1), KioskInt(r5 + 1)),
            Array((config.poolNFT, 1), (anyRewardTokenId, rewardTokenAmt - 10)),
            creationHeight = Some(newHeight)
          ),
          KioskBox(refreshAddress, minStorageRent, Array.empty, Array((refreshNFT, 1))),
          KioskBox(contracts.oracleAddress, minStorageRent, Array(pubKey1), Array((oracleTokenId, 1), (anyRewardTokenId, 16))),
          KioskBox(contracts.oracleAddress, minStorageRent, Array(pubKey2), Array((oracleTokenId, 1), (anyRewardTokenId, 21))),
          KioskBox(contracts.oracleAddress, minStorageRent, Array(pubKey3), Array((oracleTokenId, 1), (anyRewardTokenId, 31))),
          KioskBox(contracts.oracleAddress, minStorageRent, Array(pubKey4), Array((oracleTokenId, 1), (anyRewardTokenId, 41))),
          KioskBox(contracts.oracleAddress, minStorageRent, Array(pubKey5), Array((oracleTokenId, 1), (anyRewardTokenId, 51)))
        ),
        fee = 1500000,
        changeAddress,
  ...
  property("Update pool box") {
    ...
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      refresh(
        ...
        bootstrapRefreshBox(),
        rewardTokenId,
        config.oracleTokenId,
        config.refreshNFT
      )
      updatePoolBox(refresh(bootstrapPoolBox(0, 1, 0), bootstrapRefreshBox(), rewardTokenId, config.oracleTokenId, config.refreshNFT))
      ...
      def updatePoolBox(poolBox: InputBox) = {
        ...
        noException should be thrownBy
          TxUtil.createTx(
            ...
            Array(
              KioskBox(
                newPool.poolAddress,
                config.minStorageRent,
                Array(KioskLong(1), KioskInt(1)),
                Array((config.poolNFT, 1), (rewardTokenId, 1000000L)),
                creationHeight = Some(oldCreationHeight)
              ),
        ...
        // should fail when wrong creation height in pool box
        the[Exception] thrownBy
          TxUtil.createTx(
            Array(
              poolBox,
            ...
            Array(
              KioskBox(
                ...
                Array(KioskLong(1), KioskInt(1)),
                Array((config.poolNFT, 1), (rewardTokenId, 1000000L)),
                creationHeight = Some(1) // should be oldCreationHeight
              ),
              KioskBox(contracts.updateAddress, config.minStorageRent, Array.empty, Array((config.updateNFT, 1))),
              KioskBox(contracts.ballotAddress, config.minStorageRent, Array(pubKey1), Array((config.ballotTokenId, 1))),
              KioskBox(contracts.ballotAddress, config.minStorageRent, Array(pubKey2), Array((config.ballotTokenId, 1))),
        ...
        the[Exception] thrownBy TxUtil.createTx(
          ...
          Array(
            KioskBox(
              newPool.poolAddress,
              config.minStorageRent,
              Array(KioskLong(2), KioskInt(1)), // 2 Instead of 1 in first register
              Array((config.poolNFT, 1), (rewardTokenId, 1000000L)),
              creationHeight = Some(oldCreationHeight)
            ),
            KioskBox(contracts.updateAddress, config.minStorageRent, Array.empty, Array((config.updateNFT, 1))),
        ...
        an[Exception] should be thrownBy TxUtil.createTx(
          ...
          Array(
            KioskBox(
              newPool.poolAddress,
              config.minStorageRent,
              Array(KioskLong(1), KioskInt(1), KioskCollByte("junk".getBytes)),
              Array((config.poolNFT, 1), (rewardTokenId, 1000000L)),
              creationHeight = Some(oldCreationHeight)
            ),
            KioskBox(contracts.updateAddress, config.minStorageRent, Array.empty, Array((config.updateNFT, 1))),
        ...
        the[Exception] thrownBy TxUtil.createTx(
          ...
          Array(
            KioskBox(
              ...
              config.minStorageRent,
              Array(KioskLong(1), KioskInt(1)),
              Array((junkTokenId, 1), (rewardTokenId, 1000000L)),
              creationHeight = Some(oldCreationHeight)
            ),
            KioskBox(contracts.updateAddress, config.minStorageRent, Array.empty, Array((config.updateNFT, 1))),
            KioskBox(contracts.ballotAddress, config.minStorageRent, Array(pubKey1), Array((config.ballotTokenId, 1))),
  ...
  def update(oldPoolBox: InputBox,
             oldUpdateBox: InputBox,
             newPoolErgoTree: Values.ErgoTree,
             ballotAddress: String,
             ballotTokenId: String,
             optRewardTokenId: Option[String] = None,
             optRewardTokenAmt: Option[Long] = None)(implicit ctx: BlockchainContext) = {
    val votes = vote(newPoolErgoTree, ballotAddress, ballotTokenId, optRewardTokenId, optRewardTokenAmt)
    val votedBox1 = votes(0)
    val votedBox2 = votes(1)
    val votedBox3 = votes(2)
    val votedBox4 = votes(3)
    ...
    TxUtil.createTx(
      ...
      Array(
        KioskBox(
          newPoolAddress,
          config.minStorageRent,
          Array(KioskLong(r4), KioskInt(r5)),
          Array((config.poolNFT, 1), (optRewardTokenId.getOrElse(rewardTokenId), optRewardTokenAmt.getOrElse(1000000L))),
          creationHeight = Some(oldCreationHeight)
        ),
  ...
  // refresh of new pool box using same oracle tokens and same reward tokens should be possible
  property("Refresh after Update (same oracle and same reward tokens)") {
    ...
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val newRefreshBox = bootstrapRefreshBox(Some(newPool.refreshAddress), Some(newConfig.refreshNFT))

      noException should be thrownBy
        refresh(newPoolBox, newRefreshBox, rewardTokenId, config.oracleTokenId, newConfig.refreshNFT)
    }
  }

  // refresh of new pool box using same oracle tokens and different reward tokens should be possible
  property("Refresh after Update (same oracle and different reward tokens)") {
    val newConfig = OraclePool.poolConfig.copy(refreshNFT = newRefreshNFT, minDataPoints = 3)
    val newPool = new Contracts(newConfig)
    ...
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      val oldPoolBox = bootstrapPoolBox(0, 1, 0)
      val updateBox = bootstrapUpdateBox(Some(1))
      val updateTx = update(oldPoolBox, updateBox, newPool.poolErgoTree, contracts.ballotAddress, config.ballotTokenId, Some(newRewardTokenId), Some(1000L))
      val newPoolBox = updateTx.getOutputsToSpend.get(0)
      val newRefreshBox = bootstrapRefreshBox(Some(newPool.refreshAddress), Some(newConfig.refreshNFT))
  ...
  // refresh of new pool box using different oracle tokens and same reward tokens should be possible
  property("Refresh after Update (different oracle and same reward tokens)") {
    ...
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      noException should be thrownBy refresh(newPoolBox, newRefreshBox, rewardTokenId, newConfig.oracleTokenId, newConfig.refreshNFT)
    }
  }

  // refresh of new pool box using different oracle tokens and different reward tokens should be possible
  property("Refresh after Update (different oracle and different reward tokens)") {
    val newConfig = OraclePool.poolConfig.copy(refreshNFT = newRefreshNFT, minDataPoints = 3, oracleTokenId = newOracleTokenId)
    val newPool = new Contracts(newConfig)
