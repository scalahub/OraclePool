...
class ContextVarSpec extends PropSpec with Matchers with HttpClientTesting {
...
//  with HttpClientTesting {
  ...
  def createTestInput(ctx: BlockchainContext): InputBox = {
    ctx.newTxBuilder.outBoxBuilder
      ...
      .contract(ctx.compileContract(
        ...
        """{
         |  val v2 = getVar[Int](1).get
         |  val v20 = getVar[BigInt](10).get
         |  sigmaProp(v2.toBigInt == v20)
         |}""".stripMargin
      ))
      .build()
  ...
  property("InputBox support context variables") {
    ...
    ergoClient.execute { ctx: BlockchainContext =>
      ...
      val unsigned = txB
        .boxesToSpend(Arrays.asList(input))
        .outputs(output)
        .fee(1000000)
